...
const deploy = async () => {
    ...
    const contract = await new web3.eth.Contract(JSON.parse(contractSource.interface)).deploy({data: '0x' + contractSource.bytecode}).send({from: accounts[0]});

    console.log(`Contract deployed successfuly. Address: ${contract.options.address}`);
};
